...
class TaskSerializer(serializers.ModelSerializer):
    ...
    def validate_task_list(self, value):
        ...
        if value not in user_profile.house.lists.all():
            raise serializers.ValidationError("Tasklist provided does not belong to house for which user is member")
        return value
    ...
    class Meta:
        model = Task
        fields = ['url', 'id', 'name', 'description', 'status', 'created_at', 'compleated_on', 'created_by', 'completed_by', 'task_list', 'attachment']
        read_only_fields = ['created_at', 'created_by', 'compleated_on', 'completed_by', 'status']
...
class AttachmentSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
       ...
       if task_list not in user_profile.house.lists.all():
           raise serializers.ValidationError({"tasks":"Task provided does not belong to house fow which user is member"})
       return attrs  
    
    class Meta:
        model = Attachment
        fields = ['url', 'id', 'created_at', 'task','data']
